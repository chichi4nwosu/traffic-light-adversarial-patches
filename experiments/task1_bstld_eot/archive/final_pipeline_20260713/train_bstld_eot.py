...
def differentiable_eot_transform(patch):
    """
    Apply differentiable EOT transformations during optimization.

    The returned tensor remains connected to the learnable patch,
    so gradients flow back to delta.
    """
    if patch.dim() == 3:
    ...
    transformed = torch.clamp(
        patch * brightness.view(1, 1, 1, 1),
        0.0001,
        0.9999,
    )

    # Random in-plane rotation: -10 to +10 degrees.
    angle = torch.empty(
        transformed.shape[0],
        device=device,
        dtype=dtype,
    ...
    for epoch in tqdm(range(params["num_epochs"]), desc="Training Epochs"):
        ...
        for batch in tqdm(train_loader, desc=f"Epoch {epoch+1}", leave=False):
            ...
            for pgd_step in range(params["pgd_steps"]):
                ...
                for (x_center, bbox_width, patch_start_y) in patch_coords:
                    current_patch_size = int(
                        bbox_width
                        * float(params["patch_width_multiplier"])
                    )
                    ...
                    if params.get("transform", False):
                        eot_patch = differentiable_eot_transform(delta)
                    else:
                        eot_patch = delta
        ...
        if epoch % params["save_freq"] == 0:
            ...
            print(
                ...
                "Skipping legacy image-level attack metric; "
                "use the IoU-matched BSTLD evaluator for TMR "
                "and vanishing measurements."
            )
